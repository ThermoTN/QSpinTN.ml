...
Rslt.En(2) = GetEn_DMRG(EnV, T) * exp(H.lgnorm);
% keyboard;
EE = zeros([Para.L-1, 1]);
TE = zeros([Para.L-1, 1]);
maxEL2R = zeros([Para.L-1, 1]);
maxER2L = zeros([Para.L-1, 1]);
EEOld = zeros([Para.L-1, 1]);
for i = 1:1:Para.DMRGStepMax
    tic;
    for j = 1:1:(Para.L-2)
        [T, ~, maxEL2R(j), ~] = two_site_update_DMRG(EnV, T, j, '->-', Para);
        [ EnV ] = Update_DMRG_EnV(EnV, H, T, j, '->-');
        % fprintf('  %d, ->-\n', j)
    end
    for j = Para.L:-1:2
        [T, TE(j-1), maxER2L(j-1), ~] = two_site_update_DMRG(EnV, T, j, '-<-', Para);
        if j > 2
            [ EnV ] = Update_DMRG_EnV(EnV, H, T, j, '-<-');
        end
        % fprintf('  %d, -<-\n', j)
    ...
    EE = max(maxER2L);
    DeltaS = max(abs(EEOld-EE));
    EEOld = EE;
    % Rslt.En(i + 2) = Observe(T, H, Para);
    Rslt.En(i + 2) = GetEn_DMRG(EnV, T) * exp(H.lgnorm);
    % keyboard;
    Rslt.MaxEE = EE;
...
function [T, TE, EE, maxE] = two_site_update_DMRG(EnV, T, pos, dir, Para)
...
[U, S, V, TE, EE] = svdT(TT, 'Nkeep', Para.MCrit, 'epsilon', 1e-8);
Usize = size(U);
Vsize = size(V);
if pos == 1 || (pos == 2 && strcmp(dir, '-<-'))
    U = permute(U, [2,1]);
