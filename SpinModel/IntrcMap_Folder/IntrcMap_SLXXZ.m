function [ Intr ] = IntrcMap_SLXXZ( Para )
% Usage: [ Intr ] = IntrcMap_SLXXZ( Para ) 
% 2D Square Lattice XXZ model
% Model parameter:
...
Ly = Para.Geo.Ly;
L = Lx * Ly;
int_num = Lx * (Ly - 1) + Ly * (Lx - 1) + 2 * (Lx - 1) * (Ly - 1);
BCX = Para.Geo.BCX;
BCY = Para.Geo.BCY;

if strcmp(BCX, 'PBC')
    int_num = int_num + Ly + 2 * (Ly - 1);
end

if strcmp(BCY, 'PBC')
    int_num = int_num + Lx + 2 * (Lx - 1);
end
int_num = int_num * 3;
